...
export async function HashPassword(newUser: (CreateUserDto | UpdateUserDto) ): Promise<CreateUserDto|UpdateUserDto> {
    newUser.salt = await bcrypt.genSalt(saltRounds);
    newUser.password = await bcrypt.hash(newUser.password, newUser.salt);
    return Promise.resolve(newUser);
}

export async function MakeQuerryAgregateCourse(agregateObject: AgregateCourseObject) {
    let query = {}
