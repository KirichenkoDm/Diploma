...
let CourseService = class CourseService {
    ...
    async getCoursesByQuery(agregateObject) {
        const query = (0, utils_1.MakeQuerryAgregateCourse)(agregateObject);
        const courseData = await this.courseModel.find(query)
            .sort({ $natural: -1 })
            .skip(agregateObject.page * 10)
            .limit(10);
        if (!courseData || courseData.length == 0) {
            throw new common_1.NotFoundException('Courses which matches query not found');
        }
        return courseData;
    }
